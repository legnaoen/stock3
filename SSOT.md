...
| 2025-06-24 | [작성자]    | 모멘텀 실전/실험 가중치 파이프라인 구조 개선, 방향 dict/컬럼 일관화, UI/백엔드/CSV/리포트까지 방향(↑/↓) 분리 표시, 실전 가중치 자동 순방향 처리, Jinja2 오류 수정 | 실전/실험/리포트 완전 일관화, 운영 안전장치 적용 |
...
## [다음 개선 예정]
- 기본설정으로 되어 있는 사용지표(factors_base) 중 일부는 실제로는 '기본'이 아니어야 하므로, 추후 기본/선택 지표 구분 및 설정 방식을 개선할 예정.

---

## [2024-06-24] 모멘텀 UI 적중률/수익률 소수점 자리수 포맷팅 및 날짜별 snapshot 집계 구조 개선

### 1. UI 적중률/수익률 소수점 자리수 포맷팅
- momentum_optimization.html(Flask)에서 날짜별 모멘텀 결과표(momemtum_daily_table_html) 렌더링 시
- **적중률(hit_rate), 수익률(avg_return) 셀은 소수점 첫째자리까지만 표시**
    - app.py에서 float 변환 후 '%.1f' 포맷 적용
    - 실제 CSV/DB 데이터는 그대로, UI에서만 포맷팅
- (예시) 12.3%, -4.7% 등으로 표시됨

### 2. 날짜별 snapshot 집계 원칙
- momentum_daily_results.csv는 **각 날짜별 snapshot(스냅샷)만 집계**
    - 임시테이블/누적 SELECT/합산 금지
    - investment_opinion, momentum_analysis 테이블에서 date=해당날짜로만 조인/집계
- 매일의 추천수/적중률/수익률이 실제 그 날짜 기준으로만 표시됨

### 3. 주요 알고리즘/DB 연동 구조
- generate_momentum_daily_results.py: 날짜별로 investment_opinion + momentum_analysis를 date=해당날짜로만 조인하여 집계
- app.py: momentum_daily_results.csv를 읽어 2단 헤더 표로 렌더링, 적중률/수익률은 소수점 첫째자리까지만 포맷팅
- DB 구조: investment_opinion(신호), momentum_analysis(지표) 테이블이 날짜별로 연동됨

### 4. 재발 방지/빠른 진단법
- 누적 집계/임시테이블 사용 금지, 날짜별 snapshot만 집계
- UI 포맷팅은 app.py에서만 담당, 데이터/DB는 원본 그대로 유지
- 동일 이슈 발생시: ① app.py의 표 생성부, ② generate_momentum_daily_results.py의 집계부, ③ DB 테이블(date별 데이터) 순으로 점검
- SSOT.md 본 섹션에 모든 구조/변경/이슈/해결법을 반드시 기록
